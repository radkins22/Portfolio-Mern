...
const Skills = () => {
  ...
  return (
    <div
      className="h-auto min-h-screen w-screen bg-cover bg-center bg-no-repeat overflow-auto flex justify-center items-start pt-10"
            ...
            {icons.map((icon, index) => (
              <div key={index} className="flex justify-center items-center relative">
                ...
                <img
                  ...
                  onClick={() => handleIconClick(icon.alt)} // Handle click
                />

                {/* Text Visibility */}
                {clickedIcon === icon.alt && (
                  <div
                    className="absolute bottom-0 left-1/2 transform -translate-x-1/2 mb-2 text-white text-lg opacity-100 bg-black px-4 py-2 rounded transition-all duration-300"
                  >
                    {icon.alt}
                  </div>
                )}
